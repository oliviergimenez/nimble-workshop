...
**NIMBLE tutorial**: https://r-nimble.org/nimbleExamples/parallelizing_NIMBLE.html.

### Incomplete initialization

When you run `nimbleMCMC()` or `nimbleModel()`, you may get warnings thrown at you by NIMBLE like *'This model is not fully initialized'* or the value is NA or NaN even after trying to calculate **(see here)**. This is not necessarily an error, but it reflects missing values in model variables (incomplete initialization). In this situation, NIMBLE will initialize nodes with NAs by drawing from priors, and it will work or not. When possible, try to initialize all nodes (full initialization). The process can be a bit of a headache, but it helps understanding the model structure better. Going back to our animal survival example, let's purposedly forget to provide an initial value for `theta`:
...
Vectorization can make your code more efficient by manipulating one vector node `x[1:n]` instead of `n` nodes `x[1]`, ..., `x[n]`.
